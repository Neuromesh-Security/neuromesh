...
| **AI image pip-vendored Python Trivy Notes (not app deps)** | Trivy category `trivy-neuromesh-ai-threat-detector` on `neuromesh-ai-threat-detector` (`python:3.12-slim-bookworm`). Alerts **#146** (msgpack 1.1.2 / GHSA-6v7p-g79w-8964), **#147/#148** (setuptools 70.3.0 / CVE-2025-47273, CVE-2026-59890) resolve to packages listed in **pip’s** `pip/_vendor/vendor.txt` after `pip>=26.1.2` upgrade — **not** `apps/ai-threat-detector/requirements.txt` (`confluent-kafka` + `networkx` only; app never imports msgpack/setuptools). Primary acceptance: not independently controllable without a newer pip that vendors patched trees (msgpack 1.2.1 on pip main / expected 26.3+; setuptools remains a pkg_resources-only vendor pin per pip upstream). **#148** is MANIFEST.in/macOS sdist → **N/A** on Linux runtime. **#147** needs `PackageIndex.download` — not the vendored surface pip ships. Additional context: Slow Path AI/GNN is scaffold / threat-model out-of-scope — not on live k3s Fast Path (compose / experimental-ci / `validate_gnn_detection.sh` only). Tracking: [#201](https://github.com/Neuromesh-Security/neuromesh/issues/201). |
