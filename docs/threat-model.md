...
| Agent tampering by root | High → **Partially mitigated (Phase 1 + Phase 2)** | **Does not close Issue #44 completely** — residual documented below. **Phase 1 (PR #62):** Cosign-static-key signed bytecode manifest verified fail-closed at startup *before* any BPF load — three embedded objects (`sys_exec.bpf.o`, `network_filter.bpf.o`, LSM enforcement ELF). **Agent binary:** not in that manifest (circular by design). **PR #72:** pinned LSM link + deny maps so exit/crash does not drop enforcement. **Phase 2 (PR #74):** periodic runtime integrity monitor (`NEUROMESH_INTEGRITY_INTERVAL_SECS`, default 45s, clamped 30–60) re-hashes **`/proc/self/exe` only** against a startup baseline (or `NEUROMESH_AGENT_EXE_DIGEST`), confirms pinned LSM link via `PinnedLink::from_pin` + `FdLink::info()`, and confirms deny-map pins still exist. Failures increment `agent_integrity_failure_total{reason=exe_digest\|lsm_link\|pinned_map}` and emit `tracing::error!` (`neuromesh::integrity`); default **alert + exit** (`NEUROMESH_INTEGRITY_EXIT_ON_FAILURE=true`, override to alert-only). **Known residual (Phase 2 design report):** exe-digest check opens `/proc/self/exe` and therefore hashes the **running (exec’d) inode**. An `unlink`+replace of the on-disk install path leaves `/proc/self/exe` pointing at the original (often `(deleted)`) inode, so the periodic check still matches while the **path a future restart would load** is attacker-controlled. The design’s recommended dual check — (a) `/proc/self/exe` *and* (b) resolved on-disk install-path digest — is **not** implemented; only (a) + attach/map liveness shipped. Phase 1 still catches a bad next-start *if* the signed bytecode manifest/sig remain honest. Does **not** stop a determined root who also controls the alert channel or re-signs with a stolen key — tamper-*evidence*, not prevention. Live droplet tamper scenario (`scripts/manual_verify_runtime_integrity.sh`) gates final merge approval for the shipped checks. | Dragan Flavius (@DraganFlavius) | Tracked in [#44](https://github.com/Neuromesh-Security/neuromesh/issues/44) — Phase 1 + PR #72 + partial Phase 2; on-disk path re-hash still open |
