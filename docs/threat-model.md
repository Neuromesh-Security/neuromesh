...
|------|----------|-------|-------|--------|
| C execve tracepoint argv capture | Medium → **Mitigated ([#46](https://github.com/Neuromesh-Security/neuromesh/issues/46))** | Capped argv capture landed in `ExecEvent` schema v2: **8 slots × 32 bytes** (256 B total) via `bpf_probe_read_user` + `bpf_probe_read_user_str` on `sys_enter_execve` argv (not `bprm`); `CAPTURE_ARGV` on truncate/fault; surfaced on `CRITICAL_ALERT` JSON as `argv`. Residual: full env capture; args beyond 8/strings beyond 32 bytes truncated. | Dragan Flavius (@DraganFlavius) | Closed via [#46](https://github.com/Neuromesh-Security/neuromesh/issues/46) |
