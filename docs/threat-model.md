...
2. **`cgroup_id` recycling (Slice 2b):** Kernel cgroup IDs can be reused after a
   ...
   **not** rely on TTL expiry alone. **Slice 2b-i** implements: (1) node-local Pod
   ...
   (2) inotify cgroup-directory teardown watch (cgroupfs + systemd path layouts) as
   the primary recycle-race closer (typically &lt;1–10ms userspace reaction; see
   residual risk table). **Slice 2b-ii** (automatic cgroup↔pod↔SPIFFE insertion) is
   still required before identity exceptions are production-safe. A stale allow entry
   surviving until TTL could otherwise let a newly scheduled untrusted pod transiently
   inherit a deleted trusted pod’s recycled `cgroup_id` and its allow status.
---
...
| Phase 2 `cgroup_id` recycling | Medium → **Mitigated (2b-i path; residual window)** | **Slice 2b-i:** invalidate on Pod DELETE (node fieldSelector informer) **and** on cgroup directory teardown (inotify), with side table `cgroup_id↔(pod UID, path/inode)`. TTL alone remains insufficient (unchanged). **Residual:** userspace reaction after cgroup destroy before BPF map delete — typically **&lt;1–10ms** on a healthy node; can stretch under agent CPU starvation or inotify queue overflow (overflow triggers fail-closed forced resync + `identity_correlator_resync_total{reason=inotify_overflow}`). **Not** claimed zero-window. Auto-insert/correlation still 2b-ii. | Unassigned | Issue #92 / Slice 2b-i |
