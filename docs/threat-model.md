...
| Agent exit tore down LSM deny (unpinned link) | High → **Mitigated (PR A)** | **Resolved for survival:** previously `Lsm::attach()` held an aya `FdLink` only in-process — process exit (crash/OOM/rollout) auto-detached `bprm_check_security`. Now the link is pinned at `{NEUROMESH_BPF_PIN_ROOT}/neuromesh_lsm_exec_guard_link` and deny maps at `PATH_DENY_LIST` / `PATH_DENY_COUNT` under the same root (bpffs forbids `.` in pin basenames); pin failure aborts startup (fail-closed). **Not closed:** Phase 2 periodic integrity checker (`agent_integrity_failure_total`). | Dragan Flavius (@DraganFlavius) | Tracked in [#44](https://github.com/Neuromesh-Security/neuromesh/issues/44) |
| Agent tampering by root | High | **Phase 1:** Cosign-static-key signed bytecode manifest verified fail-closed at agent startup *before* any BPF load — covers the three embedded objects (`sys_exec.bpf.o`, `network_filter.bpf.o`, LSM enforcement ELF). Manifest+sig baked into the image; public key mounted via Secret (same `NEUROMESH_COSIGN_PUBLIC_KEY_PATH` env name as the webhook). Tamper-evidence of supply-chain / scripted artifact swap — does **not** stop a determined root who also controls the alert channel, re-signs with a stolen key, or attacks in-kernel memory after load. **Agent binary:** not in the bytecode manifest (circular). CI image Cosign (PR #47) + admission webhook deploy (`deploy/kubernetes/admission/`, Issue #63). **PR A:** see row above (LSM link + deny-map pin). **Still open (Phase 2):** periodic runtime re-hash / attach-point checks and `agent_integrity_failure_total`. | Dragan Flavius (@DraganFlavius) | Tracked in [#44](https://github.com/Neuromesh-Security/neuromesh/issues/44) — Phase 1 + PR A; Phase 2 open |
