...
| Agent exit tore down LSM deny (unpinned link) | High → **Mitigated (PR A)** | **Resolved for survival:** previously `Lsm::attach()` held an aya `FdLink` only in-process — process exit (crash/OOM/rollout) auto-detached `bprm_check_security`. Now the link is pinned at `{NEUROMESH_BPF_PIN_ROOT}/neuromesh_lsm_exec_guard.link` and deny maps at `PATH_DENY_LIST` / `PATH_DENY_COUNT` under the same root; pin failure aborts startup (fail-closed). **Not closed:** Phase 2 periodic integrity checker (`agent_integrity_failure_total`). | Dragan Flavius (@DraganFlavius) | Tracked in [#44](https://github.com/Neuromesh-Security/neuromesh/issues/44) |
