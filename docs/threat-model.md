...
| Agent tampering by root | High → **Mitigated (Phase 1 + Phase 2)** | **Closes Issue #44 completely** (Phase 1 startup attestation + Phase 2 ongoing runtime integrity). **Phase 1 (PR #62):** Cosign-static-key signed bytecode manifest verified fail-closed at startup *before* any BPF load — three embedded objects (`sys_exec.bpf.o`, `network_filter.bpf.o`, LSM enforcement ELF). **Agent binary:** not in that manifest (circular by design). **PR #72:** pinned LSM link + deny maps so exit/crash does not drop enforcement. **Phase 2:** periodic runtime integrity monitor (`NEUROMESH_INTEGRITY_INTERVAL_SECS`, default 45s, clamped 30–60) re-hashes `/proc/self/exe` against a startup baseline (or `NEUROMESH_AGENT_EXE_DIGEST`), confirms pinned LSM link via `PinnedLink::from_pin` + `FdLink::info()`, and confirms deny-map pins still exist. Failures increment `agent_integrity_failure_total{reason=exe_digest\|lsm_link\|pinned_map}` and emit `tracing::error!` (`neuromesh::integrity`); default **alert + exit** (`NEUROMESH_INTEGRITY_EXIT_ON_FAILURE=true`, override to alert-only). Does **not** stop a determined root who also controls the alert channel or re-signs with a stolen key — tamper-*evidence*, not prevention. Live droplet tamper scenario (`scripts/manual_verify_runtime_integrity.sh`) gates final merge approval. | Dragan Flavius (@DraganFlavius) | Closed via [#44](https://github.com/Neuromesh-Security/neuromesh/issues/44) (Phase 1 + PR #72 + Phase 2) |
