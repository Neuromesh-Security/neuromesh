...
| **RingBuf / token-bucket loss under flood** | Attacker or fork-bomb drives execve above the in-kernel token bucket (`RATE_LIMIT_BUCKET`, BPF constant ~500k/CPU). Typical production node execve is tens to low hundreds/s ([`docs/performance-baseline.md`](performance-baseline.md) §2.3.1), so this is **not** “normal K8s load exceeds 500k.” | Events dropped by design — attacker can hide in **self-generated** flood noise (E-08) |
...
|---------|----------------|-----------------|
| **Kernel token bucket** | `RATE_LIMIT_BUCKET` per-CPU (~500k evt/s BPF constant — **fork-bomb valve**, not a production-rate SLO) in `sys_exec.bpf.c` | E-01, E-08 |
...
| Unauthenticated `GET /v1/policy-bundle` | Low → **Mitigated (Slice 0)** | **Resolved for auth:** endpoint requires shared Bearer token (`NEUROMESH_POLICY_BUNDLE_TOKEN` / `_FILE`); agent never falls back to unauthenticated sync; auth failure retains last-known-good deny maps. Residual closed for **rotation procedure** by Issue [#179](https://github.com/Neuromesh-Security/neuromesh/issues/179): dual-accept N∥N+1 + fingerprint soak metrics + human-gated retire (`scripts/manual_verify_policy_bundle_token_rotation.sh`). Credential class remains shared Bearer until SPIRE-based mTLS is operable in deploy. Identity allowlist content ships in Slice 2a (schema_version 2) over this authenticated path. **Does not** by itself prove bundle bytes were untampered — see next row. | Dragan Flavius (@DraganFlavius) | Tracked in [#55](https://github.com/Neuromesh-Security/neuromesh/issues/55), rotation [#179](https://github.com/Neuromesh-Security/neuromesh/issues/179) |
