//! Side table: `cgroup_id` ↔ `(pod_uid, cgroup_path, inode)`.
//!
//! Populated at manual seed / insert time. Both invalidation paths (Pod DELETE
//! informer and cgroup teardown watch) consult this table — never invent
//! cgroup_ids from the K8s API alone in Slice 2b-i.
