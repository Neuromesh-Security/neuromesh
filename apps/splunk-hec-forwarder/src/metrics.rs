//! Prometheus counters for HEC forwarding outcomes.

use anyhow::{Context, Result};
use prometheus::{Counter, CounterVec, Gauge, Opts, Registry};
use std::sync::Arc;

pub struct HecMetrics {
...
impl HecMetrics {
    pub fn new() -> Result<Arc<Self>> {
        ...
        let forwarded_total = Counter::with_opts(Opts::new(
            "hec_forwarded_total",
            "Alerts successfully delivered to Splunk HEC",
        ))
        .context("hec_forwarded_total")?;

        let failed_total = CounterVec::new(
            Opts::new(
                "hec_forward_failed_total",
                "Splunk HEC delivery failures; label reason=retryable|non_retryable|network",
            ),
            &["reason"],
        )
        .context("hec_forward_failed_total")?;

        let dropped_total = CounterVec::new(
            Opts::new(
                "hec_forward_dropped_total",
                "Alerts dropped before HEC send; label reason=queue_full|malformed",
            ),
            &["reason"],
        )
        .context("hec_forward_dropped_total")?;

        let queue_depth = Gauge::with_opts(Opts::new(
            "hec_forward_queue_depth",
            "Current depth of the bounded HEC send queue",
        ))
        .context("hec_forward_queue_depth")?;
